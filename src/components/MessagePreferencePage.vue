<template>
  <div>
    <div class="text-h5 text-capitalize" :class="$q.dark.isActive ? 'text-white' : 'text-dark'">message preference</div>
    <q-separator />
    <div class="q-mt-md">
    ...
    <q-dialog v-model="preferenceDialog" persistent>
      <q-card style="width: 500px; max-width: 80vw;">
        <q-card-section>
          <div class="text-h5 text-capitalize">{{isEdit ? 'modify canned message' : 'create canned message'}}</div>
        </q-card-section>
        <q-separator />
        ...
        <q-card-section>
          <q-input v-model="title" outlined :color="$q.dark.isActive ? 'white' : 'primary'" type="text" label="Title" stack-label :error-message="errors.title.msg" :error="errors.title.type" class="q-ma-xs" />
          <q-input v-model="content" autogrow outlined :color="$q.dark.isActive ? 'white' : 'primary'" type="text" label="Content" stack-label :error-message="errors.content.msg" :error="errors.content.type" class="q-ma-xs" />
        </q-card-section>
        <q-separator />
...
const columns = ref([
  ...
  {name: 'message', label: 'Message', field: '', sortable: true, align: 'left'},
...
const createPreference = async () => {
  ...
  try {
     ...
     * call to save
     */
    const res = await server.post(`api/message`, {
      title: title.value,
...
const updatePreference = async () => {
  ...
  try {
     ...
     * call to update
     */
    const res = await server.put(`api/message/${id.value}`, {
      title: title.value,
      content: content.value
    })
    /**
...
const createDisable = async () => {
  ...
  try {
     ...
     * call to disable
     */
    const res = await server.post(`api/message/disable/${id.value}`)
    /**
     * response message
     */
    Swal.fire({
...
const createEnable = async () => {
  ...
  try {
     ...
     * call to disable
     */
    const res = await server.post(`api/message/enable/${id.value}`)
    /**
     * response message
     */
    Swal.fire({
...
const getAllPreference = async () => {
  ...
  try {
    const res = await server.get(`api/message`)
...
const searchList = debounce(async (val) => {
  tableLoading.value = true
  try {
    const res = await server.get(`api/message?filter=${val}`)
    lists.value = res.data
    tableLoading.value = false
  } catch (error) {
    tableLoading.value = false
