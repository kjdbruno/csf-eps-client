<template>
  <div>
    <div class="text-h5 text-capitalize" :class="$q.dark.isActive ? 'text-white' : 'text-dark'">office preference</div>
    <q-separator />
    <div class="q-mt-md">
    ...
    <q-dialog v-model="preferenceDialog" persistent>
      <q-card style="width: 500px; max-width: 80vw;">
        <q-card-section>
          <div class="text-h5 text-capitalize">{{ isEdit ? 'modify office' : 'create office' }}</div>
        </q-card-section>
        <q-separator />
        ...
        <q-card-section>
          <q-input v-model="label" :color="$q.dark.isActive ? 'white' : 'primary'" outlined type="text" label="Label" stack-label :error-message="errors.label.msg" :error="errors.label.type" class="q-ma-xs" />
          <q-input v-model="code" :color="$q.dark.isActive ? 'white' : 'primary'" outlined type="text" label="Code" stack-label :error-message="errors.code.msg" :error="errors.code.type" class="q-ma-xs" />
        </q-card-section>
        <q-separator />
...
const columns = ref([
  ...
  {name: 'label', label: 'Label/Code', field: '', sortable: true, align: 'left'},
...
const validation = () => {
  ...
  if (code.value.length < 1) {
    errors.code.msg = 'Please enter your code'
...
const createPreference = async () => {
  ...
  try {
     ...
     * call to save
     */
    const res = await server.post(`api/office`, {
      label: label.value,
      code: code.value
    })
    /**
...
const updatePreference = async () => {
  ...
  try {
     ...
     * call to update
     */
    const res = await server.put(`api/office/${id.value}`, {
      label: label.value,
      code: code.value
    })
    /**
...
const createDisable = async () => {
  ...
  try {
     ...
     * call to disable
     */
    const res = await server.post(`api/office/disable/${id.value}`)
    /**
     * response message
     */
    Swal.fire({
...
const createEnable = async () => {
  ...
  try {
     ...
     * call to disable
     */
    const res = await server.post(`api/office/enable/${id.value}`)
    /**
     * response message
     */
    Swal.fire({
...
const getAllPreference = async () => {
  ...
  try {
    const res = await server.get(`api/office`)
...
const searchList = debounce(async (val) => {
  tableLoading.value = true
  try {
    const res = await server.get(`api/office?filter=${val}`)
    lists.value = res.data
    tableLoading.value = false
  } catch (error) {
    tableLoading.value = false
