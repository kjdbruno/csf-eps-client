<template>
    <div>
      <div class="text-h5 text-capitalize" :class="$q.dark.isActive ? 'text-white' : 'text-dark'">announcement</div>
      <q-separator />
      <div class="q-mt-md">
                ...
                <q-input outlined dense debounce="500" :color="$q.dark.isActive ? 'white' : 'primary'" v-model="filter" placeholder="Search...">
                  ...
                  <template v-slot:before>
                    <q-btn unelevated size="sm" round :color="$q.dark.isActive ? 'grey-9' : 'grey-2'" text-color="grey" icon="add" @click="newPreference">
                      <q-tooltip anchor="bottom middle" self="center middle">New Announcement</q-tooltip>
                    </q-btn>
                  </template>
                </q-input>
              </div>
          ...
          <q-card-section class="q-pa-none">
            <q-table flat :rows="lists" :columns="columns" :loading="tableLoading" no-data-label="I didn't find anything for you">
              ...
              <template v-slot:body-cell-label="props">
                <q-td :props="props">
                  <div class="text-caption my-table-details">
                    {{props.row.content}}
                  </div>
                </q-td>
              </template>
      ...
      <q-dialog v-model="preferenceDialog" persistent>
        <q-card style="width: 500px; max-width: 80vw;">
          <q-card-section>
            <div class="text-h5 text-capitalize">{{isEdit ? 'modify announcement' : 'create announcement'}}</div>
          </q-card-section>
          <q-separator />
          <q-card-section v-if="hasError" class="q-pb-none q-ma-xs">
          ...
          <q-card-section>
            <q-input autogrow v-model="label" :color="$q.dark.isActive ? 'white' : 'primary'" outlined type="text" label="Label" stack-label :error-message="errors.label.msg" :error="errors.label.type" class="q-ma-xs" />
            <input type="file" @change="onPhotoChange"  ref="file" style="display: none" />
            <div class="q-pl-xs q-pr-xs">
                <q-btn v-ripple v-if="!photo" :color="$q.dark.isActive ? 'grey-9' : 'primary'" text-color="white" unelevated label="Upload photo" class="q-mt-xs" size="xs" @click="$refs.file.click()" />
                <q-btn v-ripple v-if="photo" :color="$q.dark.isActive ? 'grey-9' : 'primary'" text-color="white" unelevated label="remove photo" class="q-mt-xs" size="xs" @click="removePhoto" />
...
const createPreference = async () => {
  ...
  try {
     ...
     * call to save
     */
    const res = await server.post(`api/announcement`, {
      label: label.value,
      photo: photo.value
    })
    /**
...
const modifyPreference = async (data) => {
  /**
   * enable edit mode, dialog, has error
   */
  isEdit.value = true
  preferenceDialog.value = true
  hasError.value = false
...
const updatePreference = async () => {
  ...
  try {
     ...
     * call to update
     */
    const res = await server.put(`api/announcement/${id.value}`, {
      label: label.value,
      photo: photo.value
    })
    /**
...
const disablePreference = async (data) => {
   ...
   */
    Swal.fire({
    title: 'Are you sure?',
    text: `Disable ${data.content}. You won't be able to revert this!`,
    icon: 'warning',
...
const createDisable = async () => {
  ...
  try {
     ...
     * call to disable
     */
    const res = await server.post(`api/announcement/disable/${id.value}`)
    /**
     * response message
     */
    Swal.fire({
...
const enablePreference = async (data) => {
   ...
   */
    Swal.fire({
    title: 'Are you sure?',
    text: `Enable ${data.content}. You won't be able to revert this!`,
    icon: 'warning',
    showCancelButton: true,
    confirmButtonColor: '#224488',
    cancelButtonColor: '#224488',
...
const createEnable = async () => {
  ...
  try {
     ...
     * call to disable
     */
    const res = await server.post(`api/announcement/enable/${id.value}`)
    /**
     * response message
     */
    Swal.fire({
...
const getAllPreference = async () => {
  ...
  try {
    const res = await server.get(`api/announcement`)
...
const searchList = debounce(async (val) => {
  tableLoading.value = true
  try {
    const res = await server.get(`api/announcement?filter=${val}`)
    lists.value = res.data
    tableLoading.value = false
  } catch (error) {
    tableLoading.value = false
