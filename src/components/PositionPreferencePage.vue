<template>
  <div>
    <div class="text-h5 text-capitalize" :class="$q.dark.isActive ? 'text-white' : 'text-dark'">position preference</div>
    <q-separator />
    <div class="q-mt-md">
    ...
    <q-dialog v-model="preferenceDialog" persistent>
      <q-card style="width: 500px; max-width: 80vw;">
        <q-card-section>
          <div class="text-h5 text-capitalize">{{ isEdit ? 'modify position' : 'create position' }}</div>
        </q-card-section>
        <q-separator />
...
const createPreference = async () => {
  ...
  try {
     ...
     * call to save
     */
    const res = await server.post(`api/position`, {
      label: label.value
    })
    /**
     * response message
...
const updatePreference = async () => {
  ...
  try {
     ...
     * call to update
     */
    const res = await server.put(`api/position/${id.value}`, {
      label: label.value
    })
    /**
     * response message
...
const createDisable = async () => {
  ...
  try {
     ...
     * call to disable
     */
    const res = await server.post(`api/position/disable/${id.value}`)
    /**
     * response message
     */
    Swal.fire({
...
const createEnable = async () => {
  ...
  try {
     ...
     * call to disable
     */
    const res = await server.post(`api/position/enable/${id.value}`)
    /**
     * response message
     */
    Swal.fire({
...
const getAllPreference = async () => {
  ...
  try {
    const res = await server.get(`api/position`)
...
const searchList = debounce(async (val) => {
  tableLoading.value = true
  try {
    const res = await server.get(`api/position?filter=${val}`)
    lists.value = res.data
    tableLoading.value = false
  } catch (error) {
    tableLoading.value = false
