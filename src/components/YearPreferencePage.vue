<template>
  <div>
    <div class="text-h5 text-capitalize" :class="$q.dark.isActive ? 'text-white' : 'text-dark'">year preference</div>
    <q-separator />
    <div class="q-mt-md">
    ...
    <q-dialog v-model="preferenceDialog" persistent>
      <q-card style="width: 500px; max-width: 80vw;">
        <q-card-section>
          <div class="text-h5 text-capitalize">{{isEdit ? 'modify year' : 'create year'}}</div>
        </q-card-section>
        <q-separator />
...
const columns = ref([
  ...
  {name: 'year', label: 'Year', field: '', sortable: true, align: 'left'},
...
const createPreference = async () => {
  ...
  try {
     ...
     * call to save
     */
    const res = await server.post(`api/year`, {
      label: label.value
    })
    /**
     * response message
...
const updatePreference = async () => {
  ...
  try {
     ...
     * call to update
     */
    const res = await server.put(`api/year/${id.value}`, {
      label: label.value
    })
    /**
     * response message
...
const disablePreference = async (data) => {
   ...
   */
   Swal.fire({
    title: 'Are you sure?',
    text: `Disable ${data.year}. You won't be able to revert this!`,
    icon: 'warning',
...
const createDisable = async () => {
  ...
  try {
     ...
     * call to disable
     */
    const res = await server.post(`api/year/disable/${id.value}`)
    /**
     * response message
     */
    Swal.fire({
...
const enablePreference = async (data) => {
   ...
   */
   Swal.fire({
    title: 'Are you sure?',
    text: `Enable ${data.year}. You won't be able to revert this!`,
    icon: 'warning',
    showCancelButton: true,
    confirmButtonColor: '#224488',
    cancelButtonColor: '#224488',
...
const createEnable = async () => {
  ...
  try {
     ...
     * call to disable
     */
    const res = await server.post(`api/year/enable/${id.value}`)
    /**
     * response message
     */
    Swal.fire({
...
const getAllPreference = async () => {
  ...
  try {
    const res = await server.get(`api/year`)
...
const searchList = debounce(async (val) => {
  tableLoading.value = true
  try {
    const res = await server.get(`api/year?filter=${val}`)
    lists.value = res.data
    tableLoading.value = false
  } catch (error) {
    tableLoading.value = false
