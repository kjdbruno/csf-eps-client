<template>
  <div>
    <div class="text-h5 text-capitalize" :class="$q.dark.isActive ? 'text-white' : 'text-dark'">poll report</div>
    <q-separator />
    <q-card class="no-shadow q-mt-md" :class="$q.dark.isActive ? 'card-border-dark' : 'card-border-light'">
      <q-card-section>
        <div class="row q-col-gutter-xs">
          <div class="col-3">
            <q-date v-model="date" range color="grey" />
          </div>
          <div class="col-3">
            <div class="q-mb-md">
            ...
            <div>
              <q-btn @click="generateSummary" size="md" color="positive" unelevated label="summary" class="q-mr-xs q-mt-xs"/>
            </div>
          </div>
          <div class="col">
...
const generateSummary = async () => {
  ...
  try {
    const res = await server.get(`api/report/poll/summary?from=${from}&to=${to}`, {
